...
- 控制字段
  ...
  + 拒收 REJ
    ...
    这是一个 NAK 帧，但与适用于选择性重复 ARQ 协议中的不同。它是一个能在回退 N 帧 ARQ 协议中使用的NAK,它能在发送方计时到时前，告知发送方最后一个帧丢失或者损坏了，以此来提高程序的效率。
  ...
  + 选择性拒收 SREJ
    ...
    这是一个适用于选择性重复 ARQ 协议的 NAK 帧。在 HDLC 协议中使用术语选择性拒收而不是选择性重复。
...
- 简介
  ...
  保留无编号帧作为系统管理之用，无编号帧携带的信息用来管理链路自身
