# 计算机知识点笔记整理

## 暂存



## 关于

完成各种有关计算机知识点的笔记整理，包括但不限于

- 课内的知识点

- 课外跟完视频后，整理的笔记。

## 计划

### 2021-1-23

[webpack](J:\前端\尚硅谷\尚硅谷2020最新版Webpack5实战教程(从入门到精通))

[ES6](J:\前端\尚硅谷\尚硅谷_ES语法_ES6-ES11)

TS

LESS

[VUE](J:\前端\撩课学院_coderwhy老师)

[VUE](J:\前端\黑马程序员\黑马程序员_李鹏周\[黑马程序员]vue.js从入门到应用)

[VUE](J:\前端\尚硅谷\尚硅谷Vue实战教程(企业级vue项目，Vue项目硅谷外卖))

## 参数记录

### 图床参数

- /软件插件轮子/all

- /前端/js/js模块化

- /前端/js/TS基础

- /前端/css

- /前端/css/less

- /前端/html

- /lsy/软件每日花费

- /AcWing/算法基础课

- 数据结构/排序

  /datastructure/sortAlgorithm

- 数据结构/all

  /datastructure/all

- 计网/网络模型

  /computerNetworks/networkModel

- 计网/计网概念

  /computerNetworks/basicConception

- 计网/数据和信号

  /computerNetworks/dataAndSignals

- 计网/数字传输

  /computerNetworks/digitalTransmission

- 计网/模拟传输

  /computerNetworks/analogueTransmission

- 计网/带宽利用

  /computerNetworks/bandwidthUtilization
  
- 计网/传输介质

  /computerNetworks/transmissionMedium

- 计网/检错与纠错

  /computerNetworks/errorDetectionAndCorrection

- 计网/数据链路控制

  /computerNetworks/dataLinkControl

- 计网/差错控制

  /computerNetworks/errorControl

- 计网/多路访问

  /computerNetworks/multipleAccess

- 计网/以太网

  /computerNetworks/Ethernet

- 计网/逻辑寻址

  /computerNetworks/logicalAddressing

- 计网/网络层

  /computerNetworks/networkLayer

- 计网/传输层

  /computerNetworks/transportLayer

- 计网/应用层

  /computerNetworks/applicationLayer 

- 数据库/关系模型

  /database/relationalModel

- 数据库/关系语言

  /database/relationalLanguage

- 数据库/关系数据库理论

  /database/relationalDatabaseTheory

- 数据库/数据库系统实现技术

  /database/databaseSystemImplementationTechnology

- 离散数学/命题

  /discreteMathematics/proposition

- 离散数学/集合

  /discreteMathematics/setAndRelation

- 离散数学/函数与无限集合

  /discreteMathematics/functionsAndInfiniteSet

- 离散数学/图论

  /discreteMathematics/graphTheory

- 软件工程/all

  softwareEngineering/all

- git

  /git

- 杂乱的存储

  /clutter

- lsy/软件每日花费



## 博客参考

[Typora-Markdown 语法手册](https://www.jianshu.com/p/8522255344ac)

[数学公式：Axmath 与 LaTeX 语法](https://www.jianshu.com/p/2be11057dc5c)

## 公式

|   公式    | 含义 |
| :-------: | :--: |
|   \quad   | 空格 |
| \frac{}{} | 相比 |
|           |      |

## 考试

### 离散数学

+ 逻辑：翻译，符号化（谓词逻辑、命题）

  全称量词 蕴涵 所有人都会死

  存在量词 且 有些人不怕死

  尽管 但是，虽然 但是 -> 且

  可兼或 不可兼或

+ 证明

  消掉量词，命题证明方法

  全称/存在量词 先消去存在（特指，全称就能对应）

+ 范式

  主析取范式 真值为真的指派

  表示

+ 集合与关系

  幂集

  笛卡尔积

  容斥原理

  关系 *占比大

  关系的特性 5个 自反 反自反 对称 反对称 传递 

  空集上的空关系 5个特性都有

  非空上的空关系 少了某个特性

  关系的运算 复合 求逆

  特殊关系 次序关系

  偏序关系-自反 反对称 传递 哈斯图

  特殊元素 极大 极小 最大 最小 上界 下界 上确界 下确界 （有限、无限-可以都不存在）

  等价关系-自反 对称 传递

  等价类-划分-等价关系 互相诱导 本质是一样的

  关系矩阵 关系图 互相诱导

+ 函数

+ 基数

  可数集

  没有最大的基数，都小于它的幂集集合的基数

+ 图论

  基本概念 连通性

  握手定理 结点读书是边数的两倍

  欧拉图，汉密尔顿图 - 构造、判定

  平面图 定义、欧拉公式

  着色 2-着色 着色数-最少的着色颜色

  二部图

  矩阵表示 最基本的

  树 算法 最小生成树 最优树-哈夫曼编码

题型：构造题-画个图、关系、欧拉、二部等等、填空选择、计算、证明

### lsy离散

• 逻辑
命题逻辑 谓词逻辑符号化
可间或 
 不可间或「不能直接用析取词」 P∨非Q
• 证明  
  谓词逻辑 含有量词  四大定理
  先消除量词  先消除存在 再消除全称
• 主析取 主合取
  下标互补

第二部分
 • 集合和关系
 • 求幂运算 所有子集 2∧n
 • 笛卡尔积 差集 容斥原理
☐ 什么是关系
☐ 什么是二元关系
☐ 可以构造多少不一样的二元关系
☐ 关系的特性  五个特性  
 • 空集上的空关系 五个特性都有
 • 非空集上的空关系 少了  特性
☐ 关系的运算 病娇不差
☐ 特有 复合运算 关系的求逆
☐ 特殊关系 等价关系 次序关系
☐ 偏序  自反 反对称 传递
☐ 哈斯图 没有子回路 没有三角形
☐ 特殊元素  8种 极大极小 最大最小 最大不一定存在
☐ 等价类
☐ 等价关系和划分可以互相诱导 
 • 关系矩阵
 • 函数
☐ 反射 单射 双射
☐ 多少个单反双数
☐ 复合运算 求逆「只有双射」

 • 集数
☐ 可数有限集 可数无限集

填空选择证明构造
 • 构造 画图 

欧拉 哈密尔顿 必考
欧拉公式 节点-面数+边数=2

着色图 二部图 着色2

 • 树
☐ 最小生成树
☐ 最优树 哈夫曼编码
       

### 计网

计网
50选择
50大题

不认识单词打星号 可以问

卷面70
实验15
平时15
 • 第一部分
☐ osi模型
  7层 名字 作用
☐ tcp ip 协议簇
  划分依据 作用
  浏览邮件 涉及应用服务有哪些
   依据网络层 原网络地址出发，找到目的地址

 • 第二部分 物理层
☐ 数据是数字 数字信号 做过作业
☐ 除了传统以太网 千兆网 编码是什么 多电平再跳转  
☐ 4B5B编码用处
☐ 数字数据模拟信号 psk fsk qam  怎么表示 有什么特点  计算
☐ 编码机制
☐ 模拟数据数字信号 采样量化编码
☐ 信道容量计算  奈奎斯特 香农
☐ 复用 平分ADSL 模拟载波系统 ， 时分 数字载波系统 了解系统原理 参数会推倒
每秒8000帧
☐ 扩频  了解
☐ 传输介质 自学 比较 谁厉害 哪方面厉害

 • 第三部分 数据链路层
☐ 差错检错 纠错码「了解」
☐ cs检验 校验和计算
☐ 滑动窗口协议 ARQ的三种
☐ 三种 hdlc 帧类型 控制字段划分
☐ 以太帧的帧结构 TCP首部  还有一个
☐ 三种帧起什么作用
☐ csma/ cd 每个细节清楚  作业
☐ 以太帧交换的原理 原路由网桥 生成树网桥
☐ 了解标准 快速以太网标准  如何施工 施工多长 线缆布局
☐ VLAN 是干什么的 作用
☐ 微观计算 传输时间 传播时间 做过题

 • 第四部分 网络层 重点！！
☐ ip首部 重点！！！！每一个字段 干什么 多长
☐ 重装
☐ ip地址计算
☐ 路由 子网划分
☐ ipv4协议！！！！！！
☐ 其他协议 ipv6 知道概念 知道表示 不用太了解 
☐ 两组 差错五种 查询 
☐ 静态路由 动态路由
☐ 路由算法

 • 传输层
☐ tcp 首部 各自字段 流量和差错控制机制
☐ 三次握手 拥塞控制
☐ udp了解

 • 应用层
☐ http dns 了解

### 数据库

数据库复习到书上P60页了

10单选 20分
关系代数 sql 25分
事务 20
规范化 20分
数据库设计 15分

![文件太大，根据网址去浏览器上看](https://gitee.com/twilight_h_1184651848/pic-go-img/raw/master/clutter/20201224102709.png)
![](F:\西电软工二学位\各学期课程\第1学期\数据库\划重点.png)





